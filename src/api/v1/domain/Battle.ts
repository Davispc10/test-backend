...
class Battle {
  ...
  private calculateScore(pokemon: Pokemon, opponent: Pokemon): number {
    ...
    const score =
      (pokemon.stat_total + pokemon.atk + pokemon.def + pokemon.sta) * typeBonus * evolutionBonus * legendaryBonus;

    return score;
  }

  private calculateTypeBonus(pokemon: Pokemon, opponent: Pokemon): number {
    const typeOneBonus = this.typeAdvantages[pokemon.type_one]?.includes(opponent.type_one) ? 1.5 : 1;
    const typeTwoBonus =
      pokemon.type_two && this.typeAdvantages[pokemon.type_two]?.includes(opponent.type_one) ? 1.5 : 1;
